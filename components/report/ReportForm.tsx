...
export function ReportForm({ onComplete }: ReportFormProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.title && data.description && data.reportType && data.platform) {
        setFormData((prev) => ({
          ...prev,
          title: data.title,
  ...
  // function to generate report id using timestamp and random bytes for tracking
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 h-auto">
      ...
      <div>
        ...
        <input
          type="text"
          placeholder="e.g. Facebook, Instagram, Telegram etc."
      ...
      <button
        ...
        className="w-full relative group overflow-hidden rounded-xl bg-gradient-to-br from-pink-500 to-orange-600
                 px-4 py-3.5 text-sm font-medium text-white shadow-lg
                 transition-all duration-200 hover:from-pink-700 hover:to-red-600
